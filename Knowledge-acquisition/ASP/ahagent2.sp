#const n = 2.
sorts
#food = {cereal, candybar, plum}.
#drinks = {coffee}.
#electricals = {dishwasher}.
#appliances = {coffeemaker} + #electricals.
#plates = {plate}.
#glasses = {waterglass}.
#containers = {coffeepot, coffeepot} + #plates + #glasses.
#others = {cellphone}.
...
% added during run from human cue.
-occurs(open(R,E),I) :- holds(on(E,L),I), not holds(at(R,L),I).
% added during run from human cue.
-occurs(move(R,L),I) :- holds(at(R,L),I).
% added during run from human cue.
-occurs(put(R,G,S),I) :- holds(on(S,L),I), not holds(at(R,L),I).
% ------------------------ inertial axioms ------------------------%
...
goal(I) :- holds(on(plum,kitchentable),I), holds(on(candybar,kitchentable),I).
holds(agent_at(human,livingroom_desk),0).
holds(agent_at(ahagent1,livingroom_coffeetable),0).
holds(at(ahagent2,kitchentable),0).
holds(agent_hand(human,coffeepot),0).
holds(grabbed(ahagent2,plum),0).
holds(grabbed(ahagent2,candybar),0).
holds(on(cereal,kitchentable),0).
holds(on(waterglass,livingroom_desk),0).
holds(on(plate,counter_one),0).
holds(on(coffeemaker,counter_three),0).
holds(on(cellphone,livingroom_desk),0).
holds(on(dishwasher,counter_three),0).
-holds(opened(dishwasher),0).
-holds(switchedon(dishwasher),0).
holds(switchedon(coffeemaker),0).
holds(made(coffee),0).
occurs(exo_put(human,coffeepot,livingroom_desk),0).
occurs(exo_grab(ahagent1,cereal),1).
display
occurs.
