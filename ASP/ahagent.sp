...
goal(I) :- holds(made(coffee),I), holds(on(coffeepot,livingroom_desk),I), holds(on(mug,livingroom_desk),I),holds(on(waterglass,kitchentable),I), holds(on(chips,kitchentable),I).
holds(agent_at(human,kitchen_smalltable),0).
holds(at(ahagent,kitchen_smalltable),0).
holds(in_hand(ahagent,waterglass),0).
holds(in_hand(ahagent,chips),0).
