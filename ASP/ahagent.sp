...
goal(I) :- holds(on(waterglass,kitchentable),I), holds(on(apple,kitchentable),I), holds(on(chips,kitchentable),I).
holds(at(ahagent2,kitchentable),0).
holds(agent_hand(human,apple),0).
holds(on(chips,kitchentable),0).
holds(on(waterglass,kitchentable),0).
holds(on(mug,livingroom_coffeetable),0).
...
holds(made(coffee),0).
occurs(exo_put(human,apple,kitchentable),0).
occurs(exo_grab(ahagent1,mug),1).
display
occurs.
